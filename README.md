...
```env
PRISMA_INTEGRATION_TOKEN=your_integration_token_here
```
...
*Note: Integration tokens and client secrets are stored in 1Password.*
...
**Start Claim DB Worker *(different port if also running Create DB Worker)*:**
```bash
cd claim-db-worker
...
| `PRISMA_INTEGRATION_TOKEN` | API access | Worker secrets |
| `PRISMA_CLIENT_SECRET` | OAuth secret | Claim worker secrets |
...
2. **OAuth Callbacks:** Update callback URLs in Prisma dashboard
